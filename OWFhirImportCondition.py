...
class OWFhirImportCondition(widget.OWWidget):
    name = "Import Condition FHIR Resources"
    description = "returns processed orange table for Condition FHIR resource"
    category = "FHIR Widgets"
    icon = "icons/condition_icon_no_bg.PNG"

    class Inputs:
        list_of_paths = widget.Input("Bundle Resource Paths", list)

    class Outputs:
        processed_table = widget.Output("Processed Condition Table", Table)


    def __init__(self):
        super().__init__()
        
        self.string_variables = ["condition_full_id", "condition_id","resource_clinicalStatus",
                                 "resource_verificationStatus","condition_onsetDateTime","condition_recordedDate","observation_subject_reference","condition_encounter_reference",
                                  "resource_effectiveDateTime", "resource_issued", "resource_valueQuantity_unit"]
        self.numeric_variables = []
        self.cat_variables = ["condition_code_coding_0_code", 
                              "condition_code_coding_0_display",
                              "condition_category_0_coding_0_display"]
        self.all_res = [] 
        self.all_keys = [] 
        ...
        self.upload_button = gui.button(box2, self, label="Import one or more condition resources", callback=self.selectObservations)
    ...
    def make_request(self):
        ...
        ## select the condition resouces from the set of obtained data 
        condition_resources = self.extract_Observations(res_from_request=json_results)
        ## apply the flatten dict algorithm for extracting information about each element
        processed_resources = list(map(self.flatten_dict, condition_resources))
        [self.all_res.append(resource) for resource in processed_resources]
        ## create domain and final table
        self.create_table()
    ...
    def make_cat_variables(self):
        ...
        ## for each categorical variable: all the variables are transformed in string format and a discrete variable is created explictly selecting the values it can assume 
        for column in cat_x:
    ...
    def make_domain(self):
        ...
        self.df = self.df.rename(columns={"resource_id" : "condition_id",
                                          "resource_subject_reference"              :"condition_subject_reference",
                                          "resource_encounter_reference"            :"condition_encounter_reference",
                                          "resource_code_coding_0_code"             : "condition_code_coding_0_code",
                                          "resource_code_coding_0_display"          : "condition_code_coding_0_display",
                                          "resource_category_0_coding_0_display"    :"condition_category_0_coding_0_display"
                                          })
        
        ## add column for matching the format of reference field 
        self.df["condition_full_id"] = "urn:uuid:" + self.df["condition_id"] 
        if self.addPrefix:
    ...
    def extract_Observations(self, path=None,res_from_request=None):
        ...
        try:
            ...
            idx_res = map(lambda resource: resource["resource"]["resourceType"] == "Condition", patient_resources) 
...
if __name__ == "__main__":
    widgetpreview.WidgetPreview(OWFhirImportCondition).run()
